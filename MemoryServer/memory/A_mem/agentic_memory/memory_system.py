...
def tyc_str_to_json(response: str):
    try:
...
class AgenticMemorySystem:
    ...
    def __init__(self, 
        ...
        self.llm_controller = llm_client
        self.evo_cnt = 0
    ...
    def analyze_content(self, content: str) -> Dict:            
        ...
        while True: # tyc add: retry loop for LLM call
            try:
                response = self.llm_controller.get_llm_response(prompt, schema={"type": "json_schema", "json_schema": {
                            "name": "response",
                            "schema": {
                                "type": "object",
                                "properties": {
                ...
                analysis = tyc_str_to_json(response)
                assert type(analysis) is dict, "Response must be a JSON object"
                for key in ["keywords", "context", "tags"]:
                    assert key in analysis, f"Response must contain key: {key}"
                return analysis
            except Exception as e:
                # print("### TYC ###", analysis)
                continue
    ...
    def add_note(self, content: str, time: str = None, **kwargs):
        ...
        self.retriever.add_documents([note.content])
    ...
    def consolidate_memories(self):
        ...
        for memory in self.memories.values():
            ...
            self.retriever.add_documents([memory.content])
    ...
    def find_related_memories_raw(self, query: str, k: int = 5) -> str:
        ...
        j = 0
        for i in indices:
            memory_str +=  "talk start time:" + all_memories[i].timestamp + "\nmemory content: " + all_memories[i].content + "memory context: " + all_memories[i].context + "memory keywords: " + str(all_memories[i].keywords) + "memory tags: " + str(all_memories[i].tags) + "\n"
            neighborhood = all_memories[i].links
            for neighbor in neighborhood:
                memory_str += "talk start time:" + all_memories[neighbor].timestamp + "\nmemory content: " + all_memories[neighbor].content + "memory context: " + all_memories[neighbor].context + "memory keywords: " + str(all_memories[neighbor].keywords) + "memory tags: " + str(all_memories[neighbor].tags) + "\n"
                if j >= k:
                    break
                j += 1
        return memory_str
    ...
    def process_memory(self, note: MemoryNote) -> Tuple[bool, MemoryNote]:
        ...
        try:
            ...
            while True: # tyc add: retry loop for LLM call
                try:
                    response = self.llm_controller.get_llm_response(
                        messages=prompt,
