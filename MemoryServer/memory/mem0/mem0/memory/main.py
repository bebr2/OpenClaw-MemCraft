...
class Memory(MemoryBase):
    def __init__(self, config: Optional[MemoryConfig] = None):
        ...
        try:
            ...
            logger.warning("Failed to initialize LLM provider=%s: %s", self.config.llm.provider, exc)
            self.llm = None
    ...
    def _add_to_vector_store(self, messages, metadata, filters, infer):
        ...
        try:
            ...
            print(f"LLM response for fact retrieval (async): {response}")
        ...
        if new_retrieved_facts:
            ...
            try:
                response: str = self.llm.get_llm_response(
            ...
            try:
                ...
                try:
                    ...
                    # 改用 re.search 查找文本内部的匹配项
                    match = re.search(pattern, response.strip())
                    json_str = match.group(1).strip() if match else response.strip()
                    new_memories_with_actions = json.loads(json_str)
                except Exception as e:
                    try:
                        new_memories_with_actions = json.loads(json_str)
                    except Exception as e:
    ...
    def _create_procedural_memory(self, messages, metadata=None, prompt=None):
        ...
        try:
            procedural_memory = self.llm.get_llm_response(messages=parsed_messages)
        except Exception as e:
    ...
    async def _add_to_vector_store(
        ...
        try:
            ...
            try:
                ...
                match = re.search(pattern, response.strip())
                if match:
                    new_retrieved_facts = json.loads(match.group(1).strip())["facts"]
                else:                     raise ValueError("No valid JSON found in the LLM response.")
            except Exception as e:
                try:
                    pattern = r"```json\n([\s\S]*?)\n```"
                    match = re.search(pattern, response.strip())
        ...
        if new_retrieved_facts:
            ...
            try:
                ...
                try:                   
                    ...
                    match = re.search(pattern, response.strip())
                    if match:
                        new_memories_with_actions = json.loads(match.group(1).strip())
                    else:                     raise ValueError("No valid JSON found in the LLM response.")
                except Exception as e:                     
                    try:
                        pattern = r"```json\n([\s\S]*?)\n```"
