...
def _parse_json_flexible(text):
    """Parse JSON robustly from plain/fenced text and object/array roots."""
    raw = (text or "").strip()
    if not raw:
        raise json.JSONDecodeError("empty content", raw, 0)

    fenced_match = re.match(r"^\s*```(?:json)?\s*([\s\S]*?)\s*```\s*$", raw, flags=re.IGNORECASE)
    if fenced_match:
    ...
    # Recover first plausible JSON object/array from mixed narration text.
    candidates = []
    obj_start = raw.find("{")
    arr_start = raw.find("[")
    if obj_start >= 0:
    ...
    for start, closer, _ in sorted(candidates, key=lambda x: x[0]):
    ...
    raise json.JSONDecodeError("no valid json payload found", raw, 0)
...
class OpenAIClient:
    def __init__(self, api_key, base_url=None, max_workers=5):
        ...
        self.client = None
        try:
            # Prefer an explicit httpx client to avoid openai/httpx proxy-arg incompatibilities.
            import httpx
            ...
            if "proxies" in str(e):
                print(
                    "OpenAI client compatibility warning: detected openai/httpx proxy parameter mismatch; "
                    "retrying with default client init."
                )
...
def get_embedding(text, model_name="all-MiniLM-L6-v2", use_cache=True, **kwargs):
    ...
    if model_init_key not in _model_cache:
        ...
        if 'bge-m3' in model_name.lower():
            ...
            # from sentence_transformers import SentenceTransformer
            requested_device = kwargs.get("device") or os.getenv("MEMORY_EMBED_DEVICE")
            ...
            init_kwargs["device"] = requested_device
            try:
                _model_cache[model_init_key] = SentenceTransformer(model_name, **init_kwargs)
            except Exception as e:
                # Fallback for environments without CUDA support.
                if str(requested_device).lower().startswith("cuda"):
                    print(f"Warning: failed to init SentenceTransformer on {requested_device}: {e}. Falling back to CPU.")
...
def gpt_generate_multi_summary(text, client: OpenAIClient, model="gpt-4o-mini"):
    ...
    try:
        parsed = _parse_json_flexible(response_text)
        if isinstance(parsed, dict):
            if isinstance(parsed.get("summaries"), list):
...
def gpt_user_profile_analysis(conversation_str: str, client: OpenAIClient, model="gpt-4o-mini", existing_user_profile="None"):
    ...
    try:
        parsed = _parse_json_flexible(result_text)
        if isinstance(parsed, dict):
            return parsed
        if isinstance(parsed, list):
            return {"items": parsed}
        return {"raw_text_profile": str(parsed)}
    except json.JSONDecodeError:
