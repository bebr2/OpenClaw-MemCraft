...
GENERATE_SYSTEM_RESPONSE_SYSTEM_PROMPT_EN = (
...
GENERATE_SYSTEM_RESPONSE_SYSTEM_PROMPT_ZH = (
    "作为一名具备出色沟通习惯的沟通专家，你将在接下来的对话中全程扮演 {relationship} 的角色。\n"
    "以下是你独特的个人特质与知识储备：\n{assistant_knowledge_text}\n"
    "用户的个人资料：\n"
    "{meta_data_text}\n"
    "你的任务是根据这些特质来生成与之相符且保持一致语气的回复。\n"
)

GENERATE_SYSTEM_RESPONSE_USER_PROMPT_EN = (
    "<CONTEXT>\n"
...
GENERATE_SYSTEM_RESPONSE_USER_PROMPT_ZH = (
    "<上下文>\n"
    "结合你最近与用户的对话内容：\n"
    "{history_text}\n\n"
    "<记忆>\n"
    ...
    "{retrieval_text}\n\n"
    "<用户特征>\n"
    "在你与用户过去的交流中，你发现用户具有以下特征：\n"
    "{background}\n\n"
    "现在，请你以 {relationship} 的身份继续和用户的对话。\n"
    "用户刚刚说：{query}\n"
   #  "请你用以下格式回复用户（最多 30 个英文单词，必须使用英文）：\n"
   #  "在回答问题时，务必检查所引用信息的时间戳是否与问题的时间范围一致"
)
