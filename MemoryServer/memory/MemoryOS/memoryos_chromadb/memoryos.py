...
class Memoryos:
    ...
    def get_user_prompt(
        ...
        if lang == "en":
            ...
            history_text = "\n".join([
                f"用户输入: {qa.get('user_input', '')}\n你的输出: {qa.get('agent_response', '')} (时间: {qa.get('timestamp', '')})"
                for qa in short_term_history
            ])
        ...
        for p in retrieved_pages:
            ...
            if lang == "en":
                ...
                page_text = f"用户输入: {p.get('user_input', '')}\n你的输出: {p.get('agent_response', '')}"
                if meta_info:
                    page_text += f"\n 对话链信息: \n{meta_info}"
            
            retrieval_text_parts.append(page_text)
        retrieval_text="【Historical Memory】\n" if lang == "en" else "【历史记忆】\n"
        retrieval_text += "\n\n".join(retrieval_text_parts)
        ...
        background_context = f"【User Profile】\n{user_profile_text}\n{user_knowledge_background}" if lang == "en" else \
                             f"【用户画像】\n{user_profile_text}\n{user_knowledge_background}"

        # 6. Format retrieved Assistant Knowledge
        assistant_knowledge_text_for_prompt = "【Assistant Knowledge Base】\n" if lang == "en" else "【助理知识库】\n"
        if retrieved_assistant_knowledge:
            for ak_entry in retrieved_assistant_knowledge:
                assistant_knowledge_text_for_prompt += f"- {ak_entry['text']}\n"
        else:
        ...
        if user_conversation_meta_data:
            ...
            meta_data_text_for_prompt += "None provided for this turn." if lang == "en" else "本轮未提供信息。"

        # 8. Construct Prompts
        if lang == "en":
            system_prompt_text = prompts.GENERATE_SYSTEM_RESPONSE_SYSTEM_PROMPT_EN.format(
                relationship=relationship_with_user,
                assistant_knowledge_text=assistant_knowledge_text_for_prompt,
                meta_data_text=meta_data_text_for_prompt # Using meta_data_text placeholder for user_conversation_meta_data
            )
            
            user_prompt_text = prompts.GENERATE_SYSTEM_RESPONSE_USER_PROMPT_EN.format(
                history_text=history_text,
                retrieval_text=retrieval_text,
                background=background_context,
                relationship=relationship_with_user,
            ...
            system_prompt_text = prompts.GENERATE_SYSTEM_RESPONSE_SYSTEM_PROMPT_ZH.format(
                relationship=relationship_with_user,
                assistant_knowledge_text=assistant_knowledge_text_for_prompt,
                meta_data_text=meta_data_text_for_prompt # Using meta_data_text placeholder for user_conversation_meta_data
            )
            
            user_prompt_text = prompts.GENERATE_SYSTEM_RESPONSE_USER_PROMPT_ZH.format(
                history_text=history_text,
                retrieval_text=retrieval_text,
                background=background_context,
                relationship=relationship_with_user,
