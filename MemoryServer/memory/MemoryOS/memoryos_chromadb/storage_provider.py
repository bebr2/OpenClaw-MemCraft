...
class ChromaStorageProvider:
    ...
    def save_all_metadata(self):
        ...
        try:
            # Defensive guard: do not clobber existing non-empty metadata with an empty snapshot.
            current = self.metadata or {}
            is_current_effectively_empty = (
                not current.get("mid_term_sessions")
                and not current.get("short_term_memory")
                and not current.get("user_profiles")
                and not current.get("update_times")
            )
            if is_current_effectively_empty and os.path.exists(self.metadata_file):
                try:
                    with open(self.metadata_file, 'r', encoding='utf-8') as f:
                        existing = json.load(f)
                    is_existing_non_empty = (
                        bool(existing.get("mid_term_sessions"))
                        or bool(existing.get("short_term_memory"))
                        or bool(existing.get("user_profiles"))
                        or bool(existing.get("update_times"))
                    )
                    if is_existing_non_empty:
                        print("ChromaStorageProvider: Skip saving empty metadata to avoid overwriting existing non-empty snapshot.")
                        return
                except (OSError, json.JSONDecodeError):
                    # Fall through to normal write path if existing snapshot cannot be inspected.
                    pass
